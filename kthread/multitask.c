...
static LIST_HEAD(tasks_list);  /* sharelist_fn头 */
...
void timer_callback(struct timer_list *timer)
{
	...
	if (!list_empty(&tasks_list)) {
		...
		if (count++ % 4 == 0)
			printk("\n");
		i = list_entry(tasks_list.next, struct my_struct, list); /* 取下一个节点 */
		list_del(tasks_list.next); /* 删除节点 */
		list_len--;
...
static void share_exit(void)
{
	...
	list_for_each_safe(p, n, &tasks_list) { /* 删除所有节点，销毁链表 */
		if (count++ % 4 == 0)
			printk("\n");
